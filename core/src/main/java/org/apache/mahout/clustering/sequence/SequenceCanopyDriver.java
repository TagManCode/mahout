...
public class SequenceCanopyDriver extends AbstractJob {

  public static final String DEFAULT_CLUSTERED_POINTS_DIRECTORY = "clusteredPoints";

  private static final Logger log = LoggerFactory.getLogger(SequenceCanopyDriver.class);

  public static void main(String[] args) throws Exception {
    ToolRunner.run(new Configuration(), new SequenceCanopyDriver(), args);
  }
  ...
  private static Path buildClustersSeq(Path input, Path output,
      DistanceMeasure measure, double t1, double t2, int clusterFilter)
    throws IOException {
    SequenceCanopyClusterer clusterer = new SequenceCanopyClusterer(measure, t1, t2);
    Collection<SequenceCanopy> canopies = Lists.newArrayList();
  ...
  private static Path buildClustersMR(Configuration conf, Path input,
      ...
      ClassNotFoundException {
    conf.set(SequenceCanopyConfigKeys.DISTANCE_MEASURE_KEY, measure.getClass()
        .getName());
    conf.set(SequenceCanopyConfigKeys.T1_KEY, String.valueOf(t1));
    conf.set(SequenceCanopyConfigKeys.T2_KEY, String.valueOf(t2));
    conf.set(SequenceCanopyConfigKeys.T3_KEY, String.valueOf(t3));
    conf.set(SequenceCanopyConfigKeys.T4_KEY, String.valueOf(t4));
    conf.set(SequenceCanopyConfigKeys.CF_KEY, String.valueOf(clusterFilter));

    Job job = new Job(conf, "Canopy Driver running buildClusters over input: "
        + input);
    job.setInputFormatClass(SequenceFileInputFormat.class);
    job.setOutputFormatClass(SequenceFileOutputFormat.class);
    job.setMapperClass(SequenceCanopyMapper.class);
    job.setMapOutputKeyClass(Text.class);
    job.setMapOutputValueClass(VectorWritable.class);
    job.setReducerClass(SequenceCanopyReducer.class);
    job.setOutputKeyClass(Text.class);
    job.setOutputValueClass(ClusterWritable.class);
    job.setNumReduceTasks(1);
    job.setJarByClass(SequenceCanopyDriver.class);
  ...
  private static void clusterData(Configuration conf,
    ...
    throws IOException, InterruptedException, ClassNotFoundException {
    ClusterClassifier.writePolicy(conf, new CanopyClusteringPolicy(), canopies);
