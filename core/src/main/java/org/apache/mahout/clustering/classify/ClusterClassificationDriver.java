...
public final class ClusterClassificationDriver extends AbstractJob {
     ...
     * @param clusteringOutputPath           the output path of clustering ( it reads clusters-*-final file
     *                                       from here )
     * @param output                         the location to store the classified vectors
     * @param clusterClassificationThreshold the threshold value of probability distribution function from 0.0
     *                                       to 1.0. Any vector with pdf less that this threshold will not be
     *                                       classified for the cluster.
     * @param runSequential                  Run the process sequentially or in a mapreduce way.
     * @throws IOException
    ...
    private static void classifyClusterSeq(Configuration conf, Path input, Path clusters, Path output, Double clusterClassificationThreshold, boolean emitMostLikely) throws IOException {
        ...
        ClusteringPolicy policy = ClusterClassifier.readPolicy(conf, finalClustersPath(conf, clusters));
        ClusterClassifier clusterClassifier = new ClusterClassifier(clusterModels, policy);
        selectCluster(conf, input, clusterModels, clusterClassifier, output, clusterClassificationThreshold, emitMostLikely);
     ...
     *
     * @param input                          the path containing the input vector.
     * @param clusterModels                  the clusters
     * @param clusterClassifier              used to classify the vectors into different clusters
     * @param output                         the path to store classified data
     * @param clusterClassificationThreshold the threshold value of probability distribution function from 0.0
     *                                       to 1.0. Any vector with pdf less that this threshold will not be
     *                                       classified for the cluster
     * @param emitMostLikely                 emit the vectors with the max pdf values per cluster
     * @throws IOException
     */
    private static void selectCluster(Path input, List<Cluster> clusterModels, ClusterClassifier clusterClassifier, Path output, Double clusterClassificationThreshold, boolean emitMostLikely) throws IOException {
        ...
        for (Pair<Writable, VectorWritable> vw : new SequenceFileDirIterable<Writable, VectorWritable>(input, PathType.LIST, PathFilters.logsCRCFilter(), conf)) {
    ...
    private static void selectCluster(Configuration conf, Path input, List<Cluster> clusterModels, ClusterClassifier clusterClassifier, Path output, Double clusterClassificationThreshold, boolean emitMostLikely) throws IOException {
        SequenceFile.Writer writer = new SequenceFile.Writer(input.getFileSystem(conf), conf, new Path(output, "part-m-" + 0), IntWritable.class, WeightedPropertyVectorWritable.class);
        for (Pair<Writable, VectorWritable> vw : new SequenceFileDirIterable<Writable, VectorWritable>(input, PathType.LIST, PathFilters.logsCRCFilter(), conf)) {
            // Converting to NamedVectors to preserve the vectorId else its not obvious as to which point
            // belongs to which cluster - fix for MAHOUT-1410
    ...
    private static void classifyClusterMR(Configuration conf, Path input, Path clustersIn, Path output, Double clusterClassificationThreshold, boolean emitMostLikely) throws IOException, InterruptedException, ClassNotFoundException {
        ...
        conf.setFloat(ClusterClassificationConfigKeys.OUTLIER_REMOVAL_THRESHOLD, clusterClassificationThreshold.floatValue());
